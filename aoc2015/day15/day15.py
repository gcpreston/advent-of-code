...
def highest_score(ingredients: Dict[str, Dict[str, int]], ts: int) -> int:
    ...
    possible_ratios = combination_sum(len(ingredients), ts)
    print('Done.')
    biggest = 0
    ...
    for r in possible_ratios:
        named_ratio = {ingr_names[i]: r[i] for i in range(len(ingr_names))}
...
def calc_score(ingredients: Dict[str, Dict[str, int]],
    ...
    for ingr in ratios:
        ...
        ingr_properties = []
        for prop, val in ingredients[ingr].items():
            ingr_properties.append(val * ratios[ingr])
